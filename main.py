...
# Create or connect to the index
            ...
            self.llm = ChatOpenAI(model="o4-mini-2025-04-16", temperature=1)

# Instead of directly passing 'retriever', use retriever.as_retriever()
            self.rag_model = RetrievalQA.from_chain_type(
                llm= self.llm,
                retriever= self.retriever.as_retriever(
                    search_kwarg ={'k':3,
                     "filter": {"doc_name": "uploaded_document"}
                                    }
                ) # Call _retriever() method
            )
    ...
    async def upload_pdf_async(self, contents: bytes)-> dict:
        loop = asyncio.get_event_loop()
        try:
    ...
    def _process_pdf_sync(self, contents: bytes) -> dict:
       bytes_content = BytesIO(contents)
       all_texts = self.extract_text_from_pdfs(bytes_content)
   
       document_name = "uploaded_document"  # Or extract from user/file name
       batch_size = 100
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    result = await rag_services.upload_pdf_async(content)
    return result
...
async def qa_chatbot(req: QueryRequest):
    ...
    answer = await rag_services.process_query_async(req.query.strip())
